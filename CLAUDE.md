...
- `benchmark/` — separate LLM-benchmark subsystem (tasks, prompts, runners, grading). Not part of the library's compatibility promise. Three tracks: `bare` (one OpenRouter completion per task), `agentic` (manual, a coding-agent CLI), and `effort` (`claude -p` at each `--effort` level, Plan 45). Clients reach the runner through the `ChatClient` Protocol in `runner/client.py`. **The effort track's cost column is intentionally empty** — it runs on a subscription seat where `modelUsage`'s `costBasis: "list"` dollars are never billed, so `cost` is always `None` and the ceiling is rate limits. **Effort results are comparable within the effort track only:** `claude -p` has no bare-completion mode, so every invocation carries the Claude Code harness preamble and `prompt_sha256` stops describing what the model saw. Every effort record carries `preamble: "claude-code-harness"` for that reason; never put effort numbers in a table beside bare numbers. `taxonomy.py` also holds `classify_trial` (`trapped` = controls pass, edge silently wrong; `broken` = a control failed — derived at report time, never stored) and `TRAP_TO_RISK`, the `trap_category` <-> catalog `risk_types` mapping; it imports nothing from `geocase.catalog` (only `fixtures.py` may), and `tests/benchmark/test_trap_coverage.py` does the cross-check. **Editing a `prompt.md` bumps `prompt_version` in `task.yaml` and archives the old text as `prompt.v<N>.md`** — the prompt-hash pin reproduces each committed run's hash from the version it recorded. Hand-typed grader constants carry a `SOURCES` citation (`tests/benchmark/test_oracle_constants.py`).
