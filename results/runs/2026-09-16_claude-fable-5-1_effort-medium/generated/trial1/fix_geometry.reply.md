Verification was not permitted in this environment, so the module below is untested here. It uses shapely's structured `make_valid`, which unions all shell interiors (so overlapping and figure-eight lobes are kept) and subtracts holes, then keeps only the polygonal parts.
