Sandbox permissions blocked both writing a scratch file and running a test, so this module is delivered unverified by execution. The approach is standard: each ring is noded and polygonized so every face it encloses is kept, holes are subtracted per polygon, and the parts are unioned. Valid input is returned unchanged, and a `make_valid` fallback guards against empty or invalid output.
