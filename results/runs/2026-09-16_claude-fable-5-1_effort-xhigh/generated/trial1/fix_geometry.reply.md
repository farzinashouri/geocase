I wasn't able to execute Python in this sandbox (the command was denied), so the module below is checked by reasoning only, not by a test run; it uses GEOS's structural `make_valid` (non-zero fill per ring, holes subtracted, parts unioned) with a self-contained noding-plus-winding-number fallback rather than the `buffer(0)` trick, which drops one lobe of a bowtie.
