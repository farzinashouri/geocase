I wasn't able to run Python in this sandbox (execution required approval), so the module below is checked by reasoning only, not by a test run. It uses GEOS's Voronoi builder clipped to the rectangle, maps each cell back to its generator via an interior point, and falls back to direct half-plane clipping if GEOS returns anything degenerate.
