Verification note: running Python required an approval prompt that this autonomous session could not answer, so the module below was checked against the installed pyproj 3.7.2 source (array handling and `polygon_area_perimeter` semantics) rather than executed. It computes geodesic area on the WGS84 ellipsoid per ring, takes absolute values so ring orientation does not matter, subtracts holes, sums MultiPolygon parts, and creates the `Geod` lazily so import has no side effects.
