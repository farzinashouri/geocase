I could not execute the module here because running Python in this session required an approval that was not granted, so the code below is verified by reasoning against the shapely 2.1 API rather than by a test run.
