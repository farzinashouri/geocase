Both the scratch-file write and the inline test run were declined, so the module below is verified by hand-tracing the standard zones, the antimeridian, and the Norway and Svalbard boundaries, not by execution.
