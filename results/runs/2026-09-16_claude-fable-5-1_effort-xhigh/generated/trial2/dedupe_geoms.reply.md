The sandbox denied both file writes and command execution, so I could not run this against shapely here. The module below is the same code I attempted to verify. It buckets by exact bounds, which are identical for any two geometries with the same point set, then confirms duplicates with GEOS topological equality.
