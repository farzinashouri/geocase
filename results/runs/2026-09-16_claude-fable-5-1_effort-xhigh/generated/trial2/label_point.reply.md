I couldn't run a local test (the sandbox denied both the scratch-file write and the interpreter call), so the module below is unverified by execution but uses only stable shapely 2.x APIs: `shapely.ops.polylabel`, `shapely.make_valid`, and `representative_point`.
