Both the file write and the test run were declined, so the module below is verified by hand-tracing the cases (crossing, touching, collinear overlap, containment, reversed direction, degenerate points, and near-miss) rather than by execution.
