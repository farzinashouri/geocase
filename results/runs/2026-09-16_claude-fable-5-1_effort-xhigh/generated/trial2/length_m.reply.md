I couldn't execute a verification run (the sandbox blocked the test command), but the module below uses only well-established APIs: `shapely.get_coordinates` and `pyproj.Geod.line_length`, which measures each segment along the WGS84 ellipsoid with Karney's geodesic algorithm.
