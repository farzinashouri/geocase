The sandbox is denying both file writes and inline Python, so I can't execute a check here. The module below relies on shapely's GEOS Voronoi builder for the normal path and includes an exact half-plane fallback that is used whenever the GEOS output can't be matched one-to-one to the input points (e.g. degenerate/collinear inputs).
