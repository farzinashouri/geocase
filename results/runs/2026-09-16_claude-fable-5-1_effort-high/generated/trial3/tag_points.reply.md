Writing the module now: it builds a shapely STRtree over the polygons, queries all points at once with the `intersects` predicate (boundary-inclusive), and keeps the smallest polygon index per point.
