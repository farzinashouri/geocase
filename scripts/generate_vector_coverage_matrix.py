...
def _build_markdown(entries: list[object]) -> str:
    ...
    ambiguous_present = any(
        _has_any(
            text,
            ["ambiguous", "engine-dependent", "engine_dependent", "depends on engine"],
        )
        for text in texts
    )
    ...
    for label, terms in spatial_terms.items():
        spatial_status[label] = _present_required(
            any(_has_any(text, terms) for text in texts)
        )
    ...
    for label, terms in edge_terms.items():
        ...
        if hits == 0:
            ...
            label in {"Empty/null geometry behavior", "Precision/rounding artifacts"}
            and hits <= 1
        ):
    ...
    for geom in GEOMETRY_TARGETS:
        lines.append(
            f"| {geom} | {_present_required(geometry_present[geom])} | ✅ required |"
        )
    ...
    for label, _aliases in FORMAT_TARGETS:
        lines.append(
            f"| {label} | {_format_status(label, format_counts[label])} | ✅ required |"
        )
    ...
    lines.append("|---|---:|---:|")
    lines.append(
        f"| Simple baseline geometries | {_present_required(simple_present)} | ✅ required |"
    )
    lines.append(
        f"| Complex/multipart geometries | {_present_required(complex_present)} | ✅ required |"
    )
    lines.append(
        f"| Valid datasets | {_present_required(valid_present)} | ✅ required |"
    )
    lines.append(
        f"| Invalid/pathological datasets | {_present_required(invalid_present)} | ✅ required |"
    )
    lines.append(
        f"| Ambiguous / engine-dependent validity | {_present_required(ambiguous_present)} | ✅ required |"
    )
    lines.append(
        f"| Degenerate but parseable validity | {_present_required(degenerate_present)} | ✅ required |"
    )
    lines.append(
        f"| Format-limited validity | {_present_required(format_limited_present)} | ✅ required |"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate vector coverage matrix markdown."
    )
    parser.add_argument(
        "--vector-root",
        type=Path,
