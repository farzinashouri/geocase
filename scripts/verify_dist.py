...
- Artifact filename versions agree with each other, with ``geocase.__version__``,
  and -- when ``--expected-version`` is passed -- with the release tag.
"""
...
from geocase.catalog.loader import load_case_index  # noqa: E402
...
def _expected_case_files() -> list[str]:
	...
	"""
	return list(load_case_index(CASE_INDEX_PATH))
...
def _check_versions(
	...
	if wheel_version and sdist_version and wheel_version != sdist_version:
		errors.append(
			f"wheel version {wheel_version} != sdist version {sdist_version}"
		)

	import geocase

	package_version = geocase.__version__
	if wheel_version and wheel_version != package_version:
		errors.append(
			f"artifact version {wheel_version} != geocase.__version__ {package_version}"
		)
