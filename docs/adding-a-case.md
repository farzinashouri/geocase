...
[`src/geocase/catalog/risk_types.py`](https://github.com/farzinashouri/geocase/blob/main/src/geocase/catalog/risk_types.py),
...
term → cases index is on the [catalog hub](_generated/catalog/index.md), and
...
- **A risk type is a failure mode, not a label.** `format_comparison` covered 37%
  of the catalog before it was moved to `tags`; anything that describes how the
  corpus is *built* rather than what can go *wrong* belongs in `tags`.
- **No risk type is better than a vague one.** A case with no failure mode
  declares `risk_types: []`. The literal string `"none"` was used 9 times before
  this was written down, and it is the absence of a risk type spelled wrong.
...
A case that ships only a file tells a consumer *what to run against*. A case
that also ships the right answer tells them **whether they got it right**, and
that is worth considerably more: it turns the case from a smoke test into a
graded one.
...
Not every case gets them. The means are written only where the raster declares
a NoData value — without one the two means are identical and the pair says
nothing — and vector and NetCDF cases are out of scope.
