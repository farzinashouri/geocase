---
title: "North Pole Point"
description: "A WGS84 point located at the geographic north pole for validating polar edge cases, coordinate handling, and projection assumptions."
---

<!-- Generated by scripts/generate_catalog_pages.py. Do not edit by hand. -->

# North Pole Point

<div class="gc-badges">
<span class="gc-badge">vector</span><span class="gc-badge">GeoJSON</span><span class="gc-badge">Point</span><span class="gc-badge">EPSG:4326</span><span class="gc-badge">tiny</span><span class="gc-badge">bundled</span>
</div>

A WGS84 point located at the geographic north pole for validating polar edge cases, coordinate handling, and projection assumptions.

<figure class="gc-figure">
<svg class="gc-diagram" viewBox="0 0 120 80" role="img" aria-label="Point geometry of north_pole_point, rendered from the case's data" xmlns="http://www.w3.org/2000/svg"><title>Point geometry of north_pole_point, rendered from the case's data</title><rect x="1" y="1" width="118" height="78" rx="3" fill="none" stroke="var(--gc-diagram-stroke)" stroke-width="1" opacity="0.35"/><circle cx="30" cy="70" r="3" fill="var(--gc-diagram-accent)"/></svg>
<figcaption>Point geometry, rendered from the case's actual geometry. Scale is normalized to the viewport and is not comparable between cases.</figcaption>
</figure>

| Property | Value |
|---|---|
| Case ID | `north_pole_point` |
| Category | vector |
| Format | GeoJSON |
| Geometry type | Point |
| CRS | `EPSG:4326` |
| Location | North Pole &mdash; 0.00&deg;E, 90.00&deg;N &rarr; 0.00&deg;E, 90.00&deg;N |
| Test tier | unit |
| Size class | tiny |
| Storage class | bundled |
| Redistributable | yes |
| Loader | `geopandas` |
| Status | validated |

## Use this case

```python
import pytest


@pytest.mark.geocase_case("north_pole_point")
def test_north_pole_point(geocase_case) -> None:
    data = geocase_case.load()
    assert data is not None
```

## Use GeoCase in your tests

Install the complete set of vector, raster, and NetCDF dependencies:

```bash
pip install "geocase[all]"
```

[View GeoCase on PyPI](https://pypi.org/project/geocase/).

## What this case checks

Ensure point-based utilities handle the north pole without crashing or making incorrect longitude assumptions.

## Risk types covered

- [`data/coordinate_edge_case`](../risk/data-coordinate-edge-case.md)
- [`extent/polar`](../risk/extent-polar.md)

## Expected behavior

| Assertion | Expected |
|---|---|
| `expect_loadable` | yes |
| `expect_valid_geometry` | yes |
| `expect_crs` | yes |
| `expected_epsg` | `4326` |
| `expected_geometry_types` | `Point` |

## Notes

Point exactly at the geographic north pole for polar edge-case coverage.

## Required capabilities

- `load`
- `geometry-validation`
- `reprojection`

## Files

- Primary: [`geometry.geojson`](https://github.com/farzinashouri/geocase/raw/main/src/geocase/data/core/vector/special/crs/north_pole_point/geometry.geojson)
- Notes: [`notes.md`](https://github.com/farzinashouri/geocase/raw/main/src/geocase/data/core/vector/special/crs/north_pole_point/notes.md)

[Browse this case on GitHub](https://github.com/farzinashouri/geocase/tree/main/src/geocase/data/core/vector/special/crs/north_pole_point)

## Source and license

- Source: geocase-curated
- License: MIT

## Tags

`crs` `north_pole` `point` `polar` `vector`

## Related cases

- [South Pole Point](south_pole_point.md) -- `south_pole_point`
- [North Pole Polygon](north_pole_polygon.md) -- `north_pole_polygon`
- [South Pole Polygon](south_pole_polygon.md) -- `south_pole_polygon`
- [Svalbard Special Zone Polygon](svalbard_special_zone_polygon.md) -- `svalbard_special_zone_polygon`
- [Web Mercator Baseline Point](web_mercator_baseline.md) -- `web_mercator_baseline`

<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "Dataset",
  "name": "North Pole Point",
  "description": "A WGS84 point located at the geographic north pole for validating polar edge cases, coordinate handling, and projection assumptions.",
  "identifier": "north_pole_point",
  "url": "https://farzinashouri.github.io/geocase/_generated/catalog/cases/north_pole_point/",
  "isPartOf": {
    "@type": "DataCatalog",
    "name": "GeoCase",
    "url": "https://farzinashouri.github.io/geocase"
  },
  "keywords": [
    "crs",
    "data/coordinate_edge_case",
    "extent/polar",
    "north_pole",
    "point",
    "polar",
    "vector"
  ],
  "license": "MIT",
  "creator": {
    "@type": "Organization",
    "name": "geocase-curated"
  },
  "distribution": [
    {
      "@type": "DataDownload",
      "encodingFormat": "GeoJSON",
      "name": "geometry.geojson"
    }
  ],
  "spatialCoverage": {
    "@type": "Place",
    "additionalProperty": {
      "@type": "PropertyValue",
      "name": "coordinateReferenceSystem",
      "value": "EPSG:4326"
    },
    "geo": {
      "@type": "GeoShape",
      "box": "90.0 0.0 90.0 0.0"
    },
    "name": "North Pole"
  }
}
</script>
