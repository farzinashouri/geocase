---
title: "GeoTIFF NoData Small"
description: "A GeoTIFF with an explicit NoData sentinel. Statistics computed without masking NoData are silently wrong — the most common raster analysis bug."
---

<!-- Generated by scripts/generate_catalog_pages.py. Do not edit by hand. -->

# GeoTIFF NoData Small

<div class="gc-badges">
<span class="gc-badge">raster</span><span class="gc-badge">GeoTIFF</span><span class="gc-badge">EPSG:32633</span><span class="gc-badge">tiny</span><span class="gc-badge">bundled</span>
</div>

A GeoTIFF with an explicit NoData sentinel. Statistics computed without masking NoData are silently wrong — the most common raster analysis bug.

| Property | Value |
|---|---|
| Case ID | `geotiff_nodata_small` |
| Category | raster |
| Format | GeoTIFF |
| CRS | `EPSG:32633` |
| Test tier | unit |
| Size class | tiny |
| Storage class | bundled |
| Redistributable | yes |
| Loader | `rasterio` |
| Status | validated |

## Use this case

```python
import pytest


@pytest.mark.geocase_case("geotiff_nodata_small")
def test_geotiff_nodata_small(geocase_case) -> None:
    data = geocase_case.load()
    assert data is not None
```

## What this case checks

Confirm that NoData pixels are correctly masked and excluded from statistical computations (min, max, mean).

## Risk types covered

- `incorrect_statistics`
- `nan_propagation`
- [`nodata_ignored`](../risk/nodata-ignored.md)

## Expected behavior

| Assertion | Expected |
|---|---|
| `expect_loadable` | yes |
| `expect_crs` | yes |
| `expected_epsg` | `32633` |
| `expect_nodata` | yes |

## Notes

### Purpose

Single-band raster with explicit NoData sentinel (`-9999`) for masking and
statistics validation.

### What to expect

- CRS is `EPSG:32633`.
- One band (`float32`).
- NoData is set and present in pixel values.

### Typical checks

- `assert_band_count(src, 1)`
- `assert_dtype(src, "float32")`
- `assert_nodata_value(src, -9999)`
- `assert_nodata_masked(data, nodata)`

### Common failure modes

- NoData ignored in mean/min/max computations.
- NoData converted to valid values during type casting.

## Required capabilities

- `load`
- `nodata-check`
- `statistics`

## Files

- Primary: `nodata_sample.tif`
- Notes: `notes.md`

## Source and license

- Source: geocase-curated
- License: MIT

## Tags

`geotiff` `masking` `nodata` `raster`

## Related cases

- [Hole Center NoData Raster](hole_center_nodata.md) -- `hole_center_nodata`
- [COG Multispectral Small](cog_multispectral_small.md) -- `cog_multispectral_small`
- [DEM NaN NoData Small](dem_nan_nodata_small.md) -- `dem_nan_nodata_small`
- [DEM Small](dem_small.md) -- `dem_small`
- [GeoTIFF Float64 Small](geotiff_float64_small.md) -- `geotiff_float64_small`

<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "Dataset",
  "name": "GeoTIFF NoData Small",
  "description": "A GeoTIFF with an explicit NoData sentinel. Statistics computed without masking NoData are silently wrong — the most common raster analysis bug.",
  "identifier": "geotiff_nodata_small",
  "url": "https://farzinashouri.github.io/geocase/_generated/catalog/cases/geotiff_nodata_small/",
  "isPartOf": {
    "@type": "DataCatalog",
    "name": "GeoCase",
    "url": "https://farzinashouri.github.io/geocase"
  },
  "keywords": [
    "geotiff",
    "incorrect_statistics",
    "masking",
    "nan_propagation",
    "nodata",
    "nodata_ignored",
    "raster"
  ],
  "license": "MIT",
  "creator": {
    "@type": "Organization",
    "name": "geocase-curated"
  },
  "distribution": [
    {
      "@type": "DataDownload",
      "encodingFormat": "GeoTIFF",
      "name": "nodata_sample.tif"
    }
  ],
  "spatialCoverage": {
    "@type": "Place",
    "additionalProperty": {
      "@type": "PropertyValue",
      "name": "coordinateReferenceSystem",
      "value": "EPSG:32633"
    }
  }
}
</script>
