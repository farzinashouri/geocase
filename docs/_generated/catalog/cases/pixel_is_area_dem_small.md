---
title: "Pixel-Is-Area DEM"
description: "The control half of the pixel-anchor pair. Declares AREA_OR_POINT=Area, so the transform's coordinates name pixel corners. Written explicitly rather than…"
---
...
The control half of the pixel-anchor pair. Declares AREA_OR_POINT=Area, so the transform's coordinates name pixel corners. Written explicitly rather than relying on GDAL's default, which omits the tag entirely -- a silent file cannot serve as one half of a differential.
...
| Property | Value |
|---|---|
| Case ID | `pixel_is_area_dem_small` |
| Category | raster |
| Format | GeoTIFF |
| CRS | `EPSG:32633` |
| Location | Central Mediterranean (synthetic, UTM 33N) &mdash; 15.00&deg;E, 37.95&deg;N &rarr; 15.00&deg;E, 37.95&deg;N |
| Test tier | unit |
| Size class | tiny |
...
@pytest.mark.geocase_case("pixel_is_area_dem_small")
def test_pixel_is_area_dem_small(geocase_case) -> None:
...
Verify that a consumer reads AREA_OR_POINT rather than assuming one convention. Against pixel_is_point_dem_small -- identical array, identical transform -- the same pixel sits half a pixel away.
...
- Primary: [`pixel_is_area_dem_small.tif`](https://github.com/farzinashouri/geocase/raw/main/src/geocase/data/core/raster/transform_conventions/pixel_is_area_dem_small.tif)
...
<script type="application/ld+json">
{
  ...
  "@type": "Dataset",
  "name": "Pixel-Is-Area DEM",
  "description": "The control half of the pixel-anchor pair. Declares AREA_OR_POINT=Area, so the transform's coordinates name pixel corners. Written explicitly rather than relying on GDAL's default, which omits the tag entirely -- a silent file cannot serve as one half of a differential.",
  "identifier": "pixel_is_area_dem_small",
  "url": "https://farzinashouri.github.io/geocase/_generated/catalog/cases/pixel_is_area_dem_small/",
  "isPartOf": {
    "@type": "DataCatalog",
    "name": "GeoCase",
  ...
  "spatialCoverage": {
    ...
    "geo": {
      "@type": "GeoShape",
      "box": "37.94759 15.0 37.950834 15.004098"
    },
