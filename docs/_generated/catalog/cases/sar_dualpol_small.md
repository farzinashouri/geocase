---
title: "SAR Dual-Pol Small"
description: "A tiny two-band Sentinel-1 IW GRDH-like scene (VV, VH): uint16 detected-amplitude DNs (not dB, not calibrated backscatter), zero-valued border-noise colum…"
---

<!-- Generated by scripts/generate_catalog_pages.py. Do not edit by hand. -->

# SAR Dual-Pol Small

<div class="gc-badges">
<span class="gc-badge">raster</span><span class="gc-badge">GeoTIFF</span><span class="gc-badge">EPSG:32633</span><span class="gc-badge">tiny</span><span class="gc-badge">bundled</span>
</div>

A tiny two-band Sentinel-1 IW GRDH-like scene (VV, VH): uint16 detected-amplitude DNs (not dB, not calibrated backscatter), zero-valued border-noise columns, sentinel NoData 0. Generated by geocase.synth, whose constants are machine-checked against a real S1A GRD annotation.

<figure class="gc-figure">
<svg class="gc-diagram" viewBox="0 0 120 80" role="img" aria-label="Schematic raster: 2 bands, 16x16" xmlns="http://www.w3.org/2000/svg"><title>Schematic raster: 2 bands, 16x16</title><rect x="1" y="1" width="118" height="78" rx="3" fill="none" stroke="var(--gc-diagram-stroke)" stroke-width="1" opacity="0.35"/><rect x="34.0" y="20.0" width="58" height="34" rx="2" fill="var(--gc-diagram-muted)" stroke="var(--gc-diagram-stroke)" stroke-width="1.5"/><rect x="28.0" y="26.0" width="58" height="34" rx="2" fill="var(--gc-diagram-fill)" stroke="var(--gc-diagram-stroke)" stroke-width="1.5"/><line x1="42.5" y1="26.0" x2="42.5" y2="60.0" stroke="var(--gc-diagram-stroke)" stroke-width="0.5" opacity="0.4"/><line x1="57.0" y1="26.0" x2="57.0" y2="60.0" stroke="var(--gc-diagram-stroke)" stroke-width="0.5" opacity="0.4"/><line x1="71.5" y1="26.0" x2="71.5" y2="60.0" stroke="var(--gc-diagram-stroke)" stroke-width="0.5" opacity="0.4"/><line x1="28.0" y1="37.3" x2="86.0" y2="37.3" stroke="var(--gc-diagram-stroke)" stroke-width="0.5" opacity="0.4"/><line x1="28.0" y1="48.7" x2="86.0" y2="48.7" stroke="var(--gc-diagram-stroke)" stroke-width="0.5" opacity="0.4"/><rect x="29.0" y="49.7" width="12.5" height="9.3" fill="none" stroke="var(--gc-diagram-stroke)" stroke-width="1.2" stroke-dasharray="2 1.5"/></svg>
<figcaption>Schematic: 2 bands, 16x16 px, uint16, sentinel NoData. Drawn from metadata, not from the pixels.</figcaption>
</figure>

| Property | Value |
|---|---|
| Case ID | `sar_dualpol_small` |
| Category | raster |
| Format | GeoTIFF |
| CRS | `EPSG:32633` |
| Test tier | integration |
| Size class | tiny |
| Storage class | bundled |
| Redistributable | yes |
| Loader | `rasterio` |
| Status | validated |

## Use this case

```python
import pytest


@pytest.mark.geocase_case("sar_dualpol_small")
def test_sar_dualpol_small(geocase_case) -> None:
    data = geocase_case.load()
    assert data is not None
```

## What this case checks

Confirm GeoCase preserves a dual-pol VV/VH SAR scene, its band ordering, and band naming.

## Risk types covered

- [`band_loss`](../risk/band-loss.md)
- [`dtype_drift`](../risk/dtype-drift.md)
- [`incorrect_band_order`](../risk/incorrect-band-order.md)

## Expected behavior

| Assertion | Expected |
|---|---|
| `expect_loadable` | yes |
| `expect_crs` | yes |
| `expected_epsg` | `32633` |
| `expect_nodata` | yes |
| `expected_band_count` | `2` |
| `expected_dtype` | `uint16` |
| `expected_shape` | `[16, 16]` |
| `expected_nodata_value` | `0` |
| `nodata_convention` | `sentinel` |
| `expected_compression` | `deflate` |
| `expected_band_names` | `VV`, `VH` |

## Required capabilities

- `load`
- `multiband-read`
- `band-name-check`

## Files

- Primary: `sar_dualpol_small.tif`

## Source and license

- Source: geocase-synthetic
- License: MIT

## Tags

`delivery:single-file` `geography:utm` `geotiff` `product:sar` `raster` `sar`

## Related cases

- [Multispectral Sentinel-2-like Small](multispectral_s2_like_small.md) -- `multispectral_s2_like_small`
- [Optical RGB Small](optical_rgb_small.md) -- `optical_rgb_small`
- [SAR VV Small](sar_vv_small.md) -- `sar_vv_small`
- [DEM Small](dem_small.md) -- `dem_small`
- [GeoTIFF Multi-Band Small](geotiff_multiband_small.md) -- `geotiff_multiband_small`

<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "Dataset",
  "name": "SAR Dual-Pol Small",
  "description": "A tiny two-band Sentinel-1 IW GRDH-like scene (VV, VH): uint16 detected-amplitude DNs (not dB, not calibrated backscatter), zero-valued border-noise columns, sentinel NoData 0. Generated by geocase.synth, whose constants are machine-checked against a real S1A GRD annotation.",
  "identifier": "sar_dualpol_small",
  "url": "https://farzinashouri.github.io/geocase/_generated/catalog/cases/sar_dualpol_small/",
  "isPartOf": {
    "@type": "DataCatalog",
    "name": "GeoCase",
    "url": "https://farzinashouri.github.io/geocase"
  },
  "keywords": [
    "band_loss",
    "delivery:single-file",
    "dtype_drift",
    "geography:utm",
    "geotiff",
    "incorrect_band_order",
    "product:sar",
    "raster",
    "sar"
  ],
  "license": "MIT",
  "creator": {
    "@type": "Organization",
    "name": "geocase-synthetic"
  },
  "distribution": [
    {
      "@type": "DataDownload",
      "encodingFormat": "GeoTIFF",
      "name": "sar_dualpol_small.tif"
    }
  ],
  "spatialCoverage": {
    "@type": "Place",
    "additionalProperty": {
      "@type": "PropertyValue",
      "name": "coordinateReferenceSystem",
      "value": "EPSG:32633"
    }
  }
}
</script>
